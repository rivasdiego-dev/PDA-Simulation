...
export default function App() {
  ...
  const decrementStates = () => {
    if (pda.states > 1) {
      setPda({
        ...pda,
        states: pda.states - 1,
        acceptedStates: pda.acceptedStates.filter(state => state < pda.states - 1)
      });
    }
  };
  ...
  const updateInstruction = (key: string, index: number, value: string) => {
    ...
    const instruction = newInstructions.get(key) || ['', VOID_SYMBOL, VOID_SYMBOL];
  ...
  const runSimulation = () => {
      ...
      if (pop !== VOID_SYMBOL) {
        if (currentStack[currentStack.length - 1] !== pop) {
          if (currentStack[currentStack.length - 1] === STACK_BOTTOM) {
            setSimulationResult("Cadena rechazada - Intento de pop en pila vacía");
            setSimulationSteps(steps);
            setCurrentStep(0);
            return;
          }
          setSimulationResult("Cadena rechazada - Símbolo incorrecto en la cima de la pila");
          setSimulationSteps(steps);
          setCurrentStep(0);
          return;
        }
    ...
    const isAccepted = pda.acceptedStates.includes(currentState) && currentStack.length === 1 && currentStack[0] === STACK_BOTTOM;
          ...
          <Tabs defaultValue="parameters">
              ...
              <div className="grid w-full items-center gap-6">
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {pda.alphabet.map((symbol, index) => (
                      <span key={index} className="bg-secondary text-secondary-foreground px-2 py-1 rounded-md flex items-center">
                        {symbol}
                        <button onClick={() => removeSymbol(symbol)} className="ml-2 text-secondary-foreground/70 hover:text-secondary-foreground">
                          <X size={14} />
                        </button>
                      </span>
                    ))}
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="flex items-center space-x-2">
                    <Button size='icon' onClick={decrementStates} disabled={pda.states <= 1}><Minus /></Button>
                    <span className="text-lg font-semibold">{pda.states}</span>
                    <Button size='icon' onClick={incrementStates}><Plus /></Button>
                  </div>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {Array.from({ length: pda.states }, (_, i) => (
                      <Button
                        key={i}
                        variant={pda.acceptedStates.includes(i) ? "secondary" : "outline"}
                        className={`rounded-full w-10 h-10 p-0 ${pda.acceptedStates.includes(i) ? 'border-2 border-primary' : ''}`}
                        onClick={() => toggleAcceptedState(i)}
                      >
                        q{i}
                      </Button>
                    ))}
                    <Button variant="outline" className="rounded-full w-10 h-10 p-0 bg-destructive text-destructive-foreground" disabled>
                      qk
                    </Button>
                  </div>
                  <p className="text-muted-foreground text-sm mt-1">
                    Haga clic en los estados para marcarlos como aceptados. El estado qk es el estado de rechazo por defecto.
                  </p>
                </div>
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="flex space-x-2">
                    <Button onClick={generateInstructionKeys} disabled={instructionsGenerated}>Generar Claves</Button>
                    <Button onClick={resetInstructions} disabled={!instructionsGenerated}>Reiniciar Instrucciones</Button>
                  </div>
                  {instructionsGenerated && (
                    <Table>
                      ...
                      <TableBody>
                        {Array.from(pda.instructions.entries()).map(([key, instruction]) => (
                          <TableRow key={key}>
                            <TableCell>{key}</TableCell>
                            <TableCell>
                              <Select value={instruction[0]} onValueChange={(value) => updateInstruction(key, 0, value)}>
                                <SelectTrigger><SelectValue placeholder="Seleccionar estado" /></SelectTrigger>
                                <SelectContent>
                                  {Array.from({ length: pda.states }, (_, i) => (
                                    <SelectItem key={i} value={`q${i}`}>q{i}</SelectItem>
                                  ))}
                            ...
                            <TableCell>
                              <Select value={instruction[1]} onValueChange={(value) => updateInstruction(key, 1, value)}>
                                <SelectTrigger><SelectValue placeholder="Seleccionar Pop" /></SelectTrigger>
                                <SelectContent>
                                  <SelectItem value={VOID_SYMBOL}>{VOID_SYMBOL} (Vacío)</SelectItem>
                                  {pda.alphabet.map((symbol, index) => (
                                    <SelectItem key={index} value={symbol}>{symbol}</SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                            </TableCell>
                            <TableCell>
                              <Select value={instruction[2]} onValueChange={(value) => updateInstruction(key, 2, value)}>
                                <SelectTrigger><SelectValue placeholder="Seleccionar Push" /></SelectTrigger>
                                <SelectContent>
                                  <SelectItem value={VOID_SYMBOL}>{VOID_SYMBOL} (Vacío)</SelectItem>
                                  {pda.alphabet.map((symbol, index) => (
                                    <SelectItem key={index} value={symbol}>{symbol}</SelectItem>
            ...
            <TabsContent value="simulation">
              <div className="flex flex-col space-y-4">
                ...
                <Button onClick={runSimulation} disabled={!instructionsGenerated}>Iniciar Simulación</Button>
                {simulationSteps.length > 0 && currentStep < simulationSteps.length && (
                  <div className="bg-secondary p-4 rounded-md">
                    <h3 className="text-lg font-semibold mb-2">Paso {currentStep + 1} de {simulationSteps.length}</h3>
                    <div className="flex justify-center space-x-2 mb-4">
                      {Array.from({ length: pda.states }, (_, i) => (
                        <div
                          key={i}
                          className={`w-8 h-8 rounded-full flex items-center justify-center ${i === simulationSteps[currentStep].state
                    ...
                    <p>Entrada restante: {simulationSteps[currentStep].inputLeft}</p>
                    <p>Pila: {simulationSteps[currentStep].stack.join(', ')}</p>
                  </div>
                )}
                {simulationResult && (
                  <div className={`p-4 rounded-md ${simulationResult === "Cadena aceptada" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
